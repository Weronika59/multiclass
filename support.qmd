...
no_failure <- df_train_trans |> 
  filter(death == 0, hospdead == 0, sfdm2 == 0) |> 
  slice_sample(n = 500) 
...
```{r}
ds |> head()
ds_smote <- MLSMOTE(D = ds, k = 5, tableVDM = ds) #chyba bez VDM powinno być, ale nie działa bez niego
...
```{r}
model <- keras_model_sequential(input_shape = c(dim(df_train_trans)))
...
history <- model |> fit(
  x = df_train_trans[,-c(1:3)], y = df_train_trans[,c(1:3)],
  epochs = 20, 
  batch_size = 32,
...
X.test <- array_reshape(X_test, dim = c(dim(X_test)[1], dim(X_test)[2]*dim(X_test)[3]))
eval_dense <- model |> 
  evaluate(X.test, y_test)
