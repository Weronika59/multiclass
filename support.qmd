...
set.seed(44)
dane3$death <- as.numeric(dane3$death)
dane3$hospdead <- as.numeric(dane3$hospdead)
dane3$sfdm2 <- as.numeric(dane3$sfdm2)
w <- c()
...
rec <- recipe(death+hospdead+sfdm2 ~ ., data = df_train) |> 
  ...
  step_dummy(sex, dzgroup, dzclass, ca, income, race, dnr, diabetes, dementia)
...
df_train_trans$dnr_X2 <- as.numeric(df_train_trans$dnr_X2)
df_train_trans$dnr_X1 <- as.numeric(df_train_trans$dnr_X1)
...
df_test_trans$dnr_X2 <- as.numeric(df_test_trans$dnr_X2)
df_test_trans$dnr_X1 <- as.numeric(df_test_trans$dnr_X1)
...
```{r}
dt_train1 <- mldr_from_dataframe(df_train_trans, labelIndices = c(1:3)) 
dt_test <- mldr_from_dataframe(df_test_trans, labelIndices = c(1:3)) 
dt_train1$labelsets
dt_test$labelsets
```
...
Accuracy na poziomie 65%.

### Model z upsamplingiem

Aby nie przesadzić z *upsamplingiem*, zastosowany będzie również *downsampling* obserwacji z etykietami o największej częstości ("00000").

```{r}
set.seed(44)

#no_failure <- df_train_trans |> 
#  filter(death == 0, hospdead == 0) |> 
#  slice_sample(n = 500) 

df_train_trans$death |> unique()
df_train_trans$hospdead |> unique()
df_train_trans$sfdm2 |> unique()
dt_to_smote


dt_to_smote <- df_train_trans |> 
  filter(death == 1 | hospdead == 1 | sfdm2 == 1 | sfdm2 == 2 | sfdm2 == 3 | sfdm2 == 4 | sfdm2 == 5) |>
  select(hospdead, death, everything()) 

ds <- mldr_from_dataframe(dt_to_smote, labelIndices = 1:3)
ds$labelsets
```

Metoda MLSMOTE działa iteracyjnie i jedna iteracja nie doprowadza jeszcze do zbalansowania próby. Dlatego zastosuję ją 7 razy.

```{r}
ds_smote <- MLSMOTE(ds, k = 5) #chyba bez VDM powinno być, ale nie działa bez niego

for(i in 1:6){   
  ds_smote <- MLSMOTE(ds_smote, k=5, tableVDM = ds_smote) 
}

dt_train2 <- readRDS("data_smote.rds") 
dt_train2$labelsets
```
...
```{r}
br_train2 <- br(dt_train2, "SVM", seed=123)
pred_br_2 <- predict(br_train2, dt_test)

multilabel_confusion_matrix(dt_test, pred_br_2)
(eval_br_2 <- multilabel_evaluate(dt_test, pred_br_2, measures = c("example-based", "micro-based")))
```

#### Laber powerset model

```{r}
lp_train2 <- lp(dt_train2, "SVM", seed = 123)
pred_lp_2 <- predict(lp_train2, dt_test)

multilabel_confusion_matrix(dt_test, pred_lp_2)
(eval_lp_2 <- multilabel_evaluate(dt_test, pred_lp_2, measures = c("example-based", "micro-based")))
```

#### Classifier chains model

```{r}
cc_train2 <- ecc(dt_train2, "SVM", seed = 123)
pred_cc_2 <- predict(cc_train2, dt_test)

multilabel_confusion_matrix(dt_test, pred_cc_2)
(eval_cc_2 <- multilabel_evaluate(dt_test, pred_cc_2, measures = c("example-based", "micro-based")))
```
...
data.frame(eval_br_1) |> 
  bind_cols(eval_br_2, eval_lp_1, eval_lp_2, eval_cc_1, eval_cc_2) |> 
  rownames_to_column("metric") |> 
  setNames(c("Metric", "BR without MLSMOTE", "BR with MLSMOTE", "LP without MLSMOTE", "LP with MLSMOTE", "CC without MLSMOTE", "CC with MLSMOTE")) |> 
  gt() |> 
...
train_features <- df_train_trans[,-c(1:3)]
train_features |> dim()
train_targets <- df_train_trans[,c(1:3)]
train_targets |> dim()

test_features <- df_test_trans[,-c(1:3)]
test_features |> dim()
test_targets <- df_test_trans[,c(1:3)]

library(keras)

#model
inputs <- layer_input(batch_shape = list(NULL, 29))
outputs <- inputs %>%
  layer_flatten() |> 
...
#uczenie
history <- model |> fit(
  x = as_tensor(train_features, shape = c(6373, 29)), y = as_tensor(train_targets, shape = c(6373, 3)),
  epochs = 40, 
  batch_size = 32,
  validation_split = 0.2
)
saveRDS(history, "C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\history.rds")
save_model_hdf5(model, "C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\model.h5")

history <- readRDS("C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\history.rds")
```

```{r}
#history <- readRDS("zad4_dense_hist.rds")
plot(history)

#model <- load_model_hdf5("zad4_dense.h5")

eval_model <- model |> 
  evaluate(as_tensor(test_features, shape = c(2732, 29)), as_tensor(test_targets, shape = c(2732, 3))) #loss=0.32, accuracy=0.65

sqrt(eval_model[1]) #loss=0.57
```
...
eval_model <- model2 |> 
  evaluate(as_tensor(test_features, shape = c(2732, 29)), as_tensor(test_targets, shape = c(2732, 3))) #loss=0.37, mae=0.64

sqrt(eval_model[1]) #loss=0.61
```

Ponowne widoczne przeuczenie modelu.
...
eval_model <- model3 |> 
  evaluate(as_tensor(test_features, shape = c(2732, 29)), as_tensor(test_targets, shape = c(2732, 3))) #loss=0.37, acc=0.60
```

Mniejsze przeuczenie! Ale accuracy około 60% ://
...
eval_model <- model4 |> 
  evaluate(as_tensor(test_features, shape = c(2732, 29)), as_tensor(test_targets, shape = c(2732, 3))) #loss=0.42, acc=0.63
```

```{r}
### to samo co wyżej, ale większa liczba epok
outputs <- inputs |> 
  layer_flatten() |> 
  layer_dense(units = 64, activation = 'relu') |> 
  layer_dense(units = 32, activation = 'relu') |> 
...
model5 <- keras_model(inputs, outputs)
model5 |> 
  compile(
    loss = "mse",
    metrics = "accuracy",
...
saveRDS(history5, "C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\history5.rds")
save_model_hdf5(model5, "C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\model5.h5")

plot(history5)

eval_model <- model5 |> 
  evaluate(as_tensor(test_features, shape = c(2732, 29)), as_tensor(test_targets, shape = c(2732, 3))) #loss=0.31, acc=0.64
```

#### Model 5

```{r}
model6 <- keras_model_sequential(input_shape = c(NULL, 29))
...
model6 |> 
  compile(
    loss = "mse",
    metrics = "accuracy",
    optimizer = optimizer_nadam(learning_rate = 1e-4)
  )
...
saveRDS(history6, "C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\history6.rds")
save_model_hdf5(model6, "C:\\Users\\wnadw\\Desktop\\Pliki studia\\Rok 4\\II stopień\\semestr 1\\Zaawansowane metody uczenia maszynowego\\projekt\\dane UCI\\multiclass\\support2\\multiclass\\model6.h5")

plot(history6)

eval_model <- model6 |> 
  evaluate(as_tensor(test_features, shape = c(2732, 29)), as_tensor(test_targets, shape = c(2732, 3))) #loss=0.31, acc=0.65
```
